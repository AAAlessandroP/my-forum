...
function page(uid, hisData, hisPosts) {
    ...
    if (hisPosts.length)
        hisPosts.forEach(post => {
            s += `<textarea id=${post._id}> ${post.Text} </textarea> <div style="display:inline;"</div> <img onclick=goto('${post._id}') src="/goto_icon.svg" alt="see it" height="20" width="20">
        <img onclick="f('${post._id}')" src="/edit_icon.svg" alt=" edit" height="20" width="20"><div style="display:inline;"></div>
        <br><br>`
        });
    ...
    s += `
            ...
            function f(chi){
                $.post("/modificaNota", { id: ""+chi, text: $("#" + chi).val() }).then(() => {
                    $("#" + chi +" +div div:eq(0)").html("OK!").css( "background-color", "green" );
                    setTimeout(()=>{
                        $("#" + chi +" +div div:eq(0)").html("");
                    },1000)
                });
